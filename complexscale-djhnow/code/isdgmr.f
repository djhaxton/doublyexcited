...
C     .. Local Scalars ..
      ...
      INTEGER I, IELMAX, mpierr
C     .. External Functions ..
      DOUBLE PRECISION D1MACH, djhDNRM2
      EXTERNAL D1MACH, djhDNRM2
...
C
            ...
            DO 25 I = 2, N
               ...
               ENDIF
 25         CONTINUE
            call mpi_allreduce(mpi_in_place,ratmax,1,
...
C         err = ||x-TrueSolution||/||TrueSolution||(2-Norms).
            IF ( ITER.EQ.0 ) then
            !SOLNRM = DNRM2(N, SOLN, 1)
            solnrm = abs(ddot(n,soln,1,soln,1))

            call mpi_allreduce(mpi_in_place,solnrm,1,
     $      mpi_double_precision,mpi_max,mpi_comm_world,mpierr)
 ...
 50         CONTINUE
            ...
            call mpi_allreduce(mpi_in_place,dxnrm,1,
     $      mpi_double_precision,mpi_max,mpi_comm_world,mpierr)
