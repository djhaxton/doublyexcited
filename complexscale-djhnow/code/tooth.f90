...
!! nbig here = nbig in jrj/djh notes
!
!! nbig in program = alloc dim of tinv
!!
!!  call getinverse(tinv,nbig,nsmall*tfactor,nsmall,delta)

SUBROUTINE GETINVERSE(TINVOUT,NOUT,NBIG,NSMALL,DELTA)
...
INTEGER, INTENT(IN) :: NBIG, NSMALL,NOUT
REAL *8, INTENT(IN) :: DELTA
REAL *8, INTENT(OUT) :: TINVOUT(-NOUT:NOUT,-NOUT:NOUT,-NOUT:NOUT)
...
if (nbig.lt.nout) then
   print *, "nbig lt nout",nbig,nout;stop
endif
...
TINVOUT(:,:,:)=TINV(-NOUT:NOUT,-NOUT:NOUT,-NOUT:NOUT)
...
TR = 0
!$OMP PARALLEL DO PRIVATE(I,J,K,P,T1,T2,T3,R1,R2,R3) SHARED(DELTA,PI,TR)
DO K = -NSMALL,NSMALL
DO J = -NSMALL,NSMALL
DO I = -NSMALL,NSMALL
